...
export const initHeader = () => {
    ...
    const themeToggle = document.querySelector('.header__theme-toggle') as HTMLElement;
    ...
    if (mobileToggle) {
        mobileToggle.addEventListener('click', () => {
            ...
            // Animate hamburger
            const hamburger = mobileToggle.querySelector('.header__hamburger') as HTMLElement;
            if (hamburger) {
    ...
    if (themeToggle) {
        themeToggle.addEventListener('click', () => {
            ...
            // Update theme icon
            const themeIcon = themeToggle.querySelector('.header__theme-icon') as HTMLElement;
            if (themeIcon) {
    ...
    document.addEventListener('click', (e) => {
        if (isMenuOpen && !nav.contains(e.target as Node) && !mobileToggle.contains(e.target as Node)) {
            isMenuOpen = false;
            nav?.classList.remove('nav--open');
            const hamburger = mobileToggle?.querySelector('.header__hamburger') as HTMLElement;
            if (hamburger) {
